...
/// 并合并 config.yaml 中的 plugins.enabled 和 plugins.disabled 状态
///
/// 启用条件：在 plugins.enabled 列表中 且 不在 plugins.disabled 列表中
/// 默认状态：不在任一列表中的插件 = not enabled（false）
#[tauri::command]
            ...
            for entry in entries.flatten() {
                ...
                if path.is_dir() {
                    if let Some(name) = path.file_name().and_then(|n| n.to_str()) {
                        // 启用条件：在 enabled 中 且 不在 disabled 中
...
pub fn apply_hermes_plugin_selection(enabledIds: Vec<String>) -> Result<(), String> {
    ...
    // 计算新的 enabled 列表：已安装且被勾选的
    let new_enabled: Vec<serde_yaml::Value> = all_installed
        .iter()
        .filter(|id| enabled_set.contains(*id))
        .map(|id| serde_yaml::Value::String(id.clone()))
        .collect();

    // 计算新的 disabled 列表：已安装但未勾选的
    ...
    if new_enabled.is_empty() {
        ...
        plugins_mapping.insert(
            ...
            serde_yaml::Value::Sequence(new_enabled),
        );
    }
...
/// 从现有 Hermes config.yaml 导入插件启用状态
/// 返回当前已启用的插件 ID 列表（从 plugins.enabled 读取）
#[tauri::command]
